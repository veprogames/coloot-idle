...
<dialog bind:this={dialog}>
    <div>
        ...
        {#if artifact.data.hint}
            <p class="italic text-slate-300 text-sm mt-4">
                {artifact.data.hint}
            </p>
        {/if}
        {#if artifact.data.effectOperation === ArtifactEffectOperation.ADDITIVE}
            <small
                >This effect is additive and is applied <strong>before</strong> all
                multiplicative effects.</small
            >
        {/if}
    </div>
    <p class="font-semibold">
        Current: <span class="text-green-400">{artifact.effectString}</span>
    </p>
...
<button on:click={() => dialog.showModal()} class="relative w-16 h-16">
    ...
    <span
        class="absolute -bottom-1 right-0 font-bold text-xl
