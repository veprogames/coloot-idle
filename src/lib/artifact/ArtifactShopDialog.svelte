...
<dialog bind:this={dialog}>
    ...
    <div class="flex justify-center items-center gap-2">
        <Statbar width="16rem" value={shop.shopLevel % 1.0}
            >Tier {Math.floor(shop.shopLevel).toFixed(0)}</Statbar
        >
        <HelpButton>
            <p>Spend Gems to unlock higher Tier Artifacts in the Shop.</p>
        </HelpButton>
    </div>
        ...
        {#each { length: shop.maxTierAvailable } as _, tier}
            <div class="flex gap-4">
                {#each shop.availableArtifacts as artifact}
                    <button
                        on:click={() => shop.buyArtifact(artifact, tier)}
                        class="btn btn-gray w-16 h-16 flex-shrink-0 relative"
                    >
                        <ArtifactImage data={artifact} {tier} />
                        <span
    ...
    <div class="flex gap-2">
        ...
        <HelpButton>
            <p>
                Reset all Artifacts and get the Gems back. This will reset your
                Equipment and Level. You will keep your Crystal Levels.
            </p>
        </HelpButton>
    </div>
