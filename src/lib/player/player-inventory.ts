import Artifact, {
    ...
    type ArtifactCalculatedEffects,
...
export default class PlayerInventory implements SaverLoader {
    ...
    removeEquipment(equipment: Equipment) {
        this.equipment = this.equipment.filter(
            (equip: Equipment) => equipment !== equip,
        );
    }

    addArtifact(artifact: Artifact) {
        const existingArtifact = this.artifacts.find((a: Artifact) =>
            a.equals(artifact),
        );
        if (existingArtifact) {
    ...
    load(data: any): void {
        ...
        for (const equip of data.equipment) {
            this.equipment.push(
                new Equipment(equip.baseStat, equip.type, equip.tier),
            );
        }
        for (const artifact of data.artifacts) {
            const newArtifact = Artifact.from(
